create table if not exists armies (id int8 generated by default as identity, army_type varchar(255) not null, created_at timestamp with time zone, free_tokens float8 not null, heal_end timestamp with time zone, heal_start timestamp with time zone, heal_last_updated_at timestamp with time zone, hours_healed int4, hours_left_healing int4, is_healing boolean, is_paid boolean, name varchar(255), current_region varchar(255) not null, faction int8 not null, origin_claimbuild int8, stationed_at int8, primary key (id));
...
create table if not exists battles (id int8 generated by default as identity, agreed_battle_date timestamp with time zone, field_battle boolean, declared_date timestamp with time zone, name varchar(255), time_frozen_from timestamp with time zone, time_frozen_until timestamp with time zone, claimbuild_id int8, region_id varchar(255), war_id int8, primary key (id));
...
create table if not exists claimbuild_apps (id int8 generated by default as identity, version int4, applied_at timestamp with time zone not null, discord_accepted_message_link varchar(255), discord_application_message_link varchar(255) not null, last_vote_at timestamp with time zone not null, resolved_at timestamp with time zone, state varchar(255) not null, vote_count int2 not null, claim_build_type varchar(255) not null, claimbuild_name varchar(255), x int4, y int4, z int4, number_of_houses varchar(255), siege varchar(255), traders varchar(255), player_id int8 not null, owned_by_id int8 not null, region_id varchar(255) not null, primary key (id));
...
create table if not exists movements (id int8 generated by default as identity, end_time timestamp with time zone, hours_moved int4, hours_until_complete int4, hours_until_next_region int4, is_char_movement boolean, is_currently_active boolean, start_time timestamp with time zone, army_name int8, rpchar_id int8, primary key (id));
...
create table if not exists roleplay_apps (id int8 generated by default as identity, version int4, applied_at timestamp with time zone not null, discord_accepted_message_link varchar(255), discord_application_message_link varchar(255) not null, last_vote_at timestamp with time zone not null, resolved_at timestamp with time zone, state varchar(255) not null, vote_count int2 not null, character_name varchar(255), character_title varchar(255), gear varchar(255), link_to_lore varchar(255), pvp boolean not null, why_do_you_want_to_be_this_character varchar(255), player_id int8 not null, faction_id int8 not null, primary key (id));
...
create table if not exists rpchars (id int8 generated by default as identity, version int4, active boolean, gear varchar(255), heal_ends timestamp with time zone, injured boolean, is_healing boolean, link_to_lore varchar(255), name varchar(255), pvp boolean, started_heal timestamp with time zone, heal_last_updated_at timestamp with time zone, title varchar(25), bound_to int8, current_region varchar(255) not null, owner_id int8, primary key (id));
...
create table if not exists war_aggressors (war_id int8 not null, initial_party boolean, joining_date timestamp with time zone, participant_faction_id int8);
create table if not exists war_defenders (war_id int8 not null, initial_party boolean, joining_date timestamp with time zone, participant_faction_id int8);
create table if not exists wars (id int8 generated by default as identity, end_date timestamp with time zone, name varchar(255) not null, start_date timestamp with time zone not null, primary key (id));
